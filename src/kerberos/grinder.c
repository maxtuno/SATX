...
#if defined(_WIN32)
#if !defined(_WINSOCKAPI_) && !defined(_TIMEVAL_DEFINED)
struct timeval {
    long tv_sec;
    long tv_usec;
};
    ...
    if (d && lemmas[1]) { // if delete and not unit
      if ((S->reason[abs (lemmas[0])] - 1) == (lemmas - S->DB)) { // what is this check?
        if (S->mode != FORWARD_SAT) { // ignore pseudo unit clause deletion
          if (S->warning != NOWARNING) {
            if (S->verb) { printf ("\rc WARNING: ignoring deletion instruction %lli: ", (long long)(lemmas - S->DB)); printClause (lemmas); } }
          if (S->warning == HARDWARNING) exit (HARDWARNING);
          if (S->verb) { printf ("c ignoring deletion instruction %lli: ", (long long)(lemmas - S->DB)); printClause (lemmas); }
...
void printHelp ( ) {
    printf ("usage: grinder [INPUT] [<PROOF>] [<option> ...]\n\n");
  ...
  printf ("  -B          force backward re-checking of all clauses\n");
...
#ifndef GRINDER_NO_MAIN
int main(int argc, char **argv) {
    return grinder_entry(argc, argv);
}
#endif
